...
$(function(){
		...
		$('#login').form('sub').click(function(){
			if(/[\w]{2,20}/.test(trim($('#login').form('user').value())) && $('#login').form('pass').value().length >= 6){
				var _this = this;
				$('#loading').show().center(200,40);
				$('#loading p').html('正在登陆中，请稍后...');
				this.disabled = true;
				$(this).css('background','url(images/login_button.png) no-repeat right');
				ajax({
					method: 'post',
					...
					success: function(res){
						if(res == 0){    //用户名和密码正确
							...
							$(_this).css('background','url(images/login_button.png) no-repeat left');
							setCookie('user',trim($('#login').form('user').value()));
							setTimeout(function(){
								$('#success').hide();
								$('#login').hide();
								screen.animate({
								...
								$('#login').first().reset();   //重置表单
								$('#header .login').hide();
								$('#header .reg').hide();
								$('#header .info').show().html(getCookie('user'));
							},2000);
						}else if(res == 1){    //用户名和密码不正确
							$('#loading').hide();
							$('#login .info').html('用户名或者密码不正确，请重新输入！');
							_this.disabled = false;
							$(_this).css('background','url(images/login_button.png) no-repeat left');
						}
					}
		...
		$('#blog').form('sub').click(function(){
			if(trim($('#blog').form('title').value()).length <= 0 || trim($('#blog').form('content').value()).length <= 0){
				$('#blog .info').show().html('发表失败：标题或者内容不得为空！');
			}else{
				$('#blog .info').hide();
				var _this = this;
				$('#loading').show().center(200,40);
				$('#loading p').html('正在发表中，请稍后...');
				this.disabled = true;
				$(this).css('background','url(images/blog_button.png) no-repeat right');
				ajax({
					method: 'post',
					...
					success: function(res){
						...
						if(res == 1){
							...
							setTimeout(function(){
								...
								screen.animate({
									...
									fn: function(){
										...
										ajax({
											...
											success: function(res){
												$('#index .loading').hide();
												var json = JSON.parse(res);   //将php返回的字符窜转换成json对象才可以调用
												var html = '';
												for(var i=0;i<json.length;i++){
													html += '<div class="content">'+
																'<h2>'+json[i].title+'<span>'+json[i].date+'</span></h2>'+
																'<p>'+json[i].content+'</p>'+
															'</div>';
												}
												$('#index').html(html);
								...
								_this.disabled = false;
								$(_this).css('background','url(images/blog_button.png) no-repeat left');
							},2000);
						}
					}
				});
		...
		$('#header .huanfu').click(function(){
			...
			ajax({
				...
				success: function(res){
					...
					for(var i=0;i<json.length;i++){
						html += '<dl>'+
									'<dt><img src="images/'+json[i].small_bg+'" big_bg="'+json[i].big_bg+'" bg_color="'+json[i].bg_color+'" alt="'+json[i].bg_text+'" /></dt>'+
		...
		//用户名验证
		$('#reg').form('user').bind('focus',function(){
			$('.info_user').show();
			$('.error_user').hide();
			$('.success_user').hide();
		}).bind('blur',function(){
		...
		function check_user(){
			var flag = true;
			if(!/[\w]{2,20}/.test(trim($('#reg').form('user').value()))){
		...
		//密码验证
		$('#reg').form('pass').bind('focus',function(){
			$('.info_pass').show();
			$('.error_pass').hide();
			$('.success_pass').hide();
		}).bind('blur',function(){
		...
		//密码强度验证
		$('#reg').form('pass').bind('keyup',function(){
		...
		function check_pass(){
			...
			var value = trim($('#reg').form('pass').value());
		...
		//确认密码验证
		$('#reg').form('notpass').bind('focus',function(){
			$('.info_notpass').show();
			$('.error_notpass').hide();
		...
		function check_notpass(){
			var pass_value = trim($('#reg').form('pass').value());
			var notpass_value = trim($('#reg').form('notpass').value());
			if(notpass_value == pass_value) return true;
		}


		//提问
		$('#reg').form('question').bind('change',function(){
			if(check_question()){
				$('.error_question').hide();
			}
		});
		...
		function check_question(){
			if($('#reg').form('question').value() != 0) return true;
		}


		//回答验证
		$('#reg').form('answer').bind('focus',function(){
			$('.info_answer').show();
			$('.error_answer').hide();
			$('.success_answer').hide();
		}).bind('blur',function(){
		...
		function check_answer(){
			if(trim($('#reg').form('answer').value()).length >= 2 && trim($('#reg').form('answer').value()).length <= 32) return true;
		}
		...
		//电子邮件验证
		$('#reg').form('email').bind('focus',function(){
			$('.info_email').show();
		...
		function check_email(){
			if(/^[\w\-\.]+@[\w\-]+(\.[a-zA-Z]{2,4}){1,2}$/.test(trim($('#reg').form('email').value()))) return true;
		}


		//电子邮件自动补全
		$('#reg').form('email').bind('keyup',function(e){
			if($(this).value().indexOf('@') == -1){
				$('.all_email').show();
		...
		$('.all_email li').bind('mousedown',function(){
			$('#reg').form('email').value($(this).text());
			$('.all_email').hide();
		});
		...
		//备注验证
		$('#reg').form('ps').bind('keyup',function(){
			check_ps();
		});
		...
		$('#reg .clear').click(function(){
			var val = $('#reg').form('ps').value().substring(0,200);
			$('#reg').form('ps').value(val);
			check_ps();
		});


		function check_ps(){
			var num = 200 - $('#reg').form('ps').value().length;
			if(num >= 0){
				$('#reg .sr').show();
				$('#reg .cg').hide();
